    ...
    <div class="maincontent">
      <v-container>
        ...
        <v-row no-gutters v-else>
          <v-col v-for="article in articles" :key="article.contentId" sm="4">
            <div class="card" elevation="1">
              ...
              ></iframe>
              <p>{{ article.title }}</p>
              <p><v-icon>mdi-eye</v-icon> {{ article.viewCounter }}回視聴</p>
            </div>
          </v-col>
        </v-row>
      </v-container>
...
export default {
  ...
  async mounted() {
    ...
    const response = await axios
      .get(process.env.VUE_APP_API_BASEURL + "/niconico?q=" + data.a)
      .catch((error) => {
  ...
  methods: {
    search_video: async function () {
      ...
      const response = await axios.get(
        process.env.VUE_APP_API_BASEURL + "/niconico?q=" + data.a
      );
      console.log(response.data.data);
      this.articles = response.data.data;
    },
