...
exports.handler = async (event, context) => {
  ...
  const URL = `https://api.search.nicovideo.jp/api/v2/snapshot/video/contents/search?q=${encodeURIComponent(
    keyWord
  )}&targets=title&fields=contentId,title,userId,description,genre,startTime,tags,thumbnailUrl,likeCounter,mylistCounter,viewCounter,commentCounter&filters[viewCounter][gte]=10000&_sort=-viewCounter&_offset=0&_limit=${encodeURIComponent(
    requestNumber
  )}&_context=apiguide`;
  //const URL = '';
